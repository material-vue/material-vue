...
function addAlphaToColor(source, opacity) {
    const color = (parseInt(source) & 0x00FFFFFF).toString(16).padStart(6, '0')
    const _opacity = Math.round(Math.min(Math.max(opacity || 1, 0), 1) * 255);
    return color + _opacity.toString(16).toUpperCase();
}
export function applyTheme(themeLight={}, themeDark={}, element=null) {
    let styleStringDark = '.dark, [m-theme="dark"] {'
    let styleStringLight = ''
    let styleStringLightClass = ''
    ...
    styleStringLight += `--md-sys-color-primaryA12:` + "#" + addAlphaToColor((parseInt(themeLight.primary) & 0x00FFFFFF).toString(16).padStart(6, '0'), 0.12) + ';\n';
    styleStringLight += `--md-sys-color-onSurfaceA12:` + "#" + addAlphaToColor((parseInt(themeLight.onSurface) & 0x00FFFFFF).toString(16).padStart(6, '0'), 0.12) + ';\n';


    styleStringLight += '}';
    styleStringLightClass = ':root {' + styleStringLight;
    styleStringLight = '.light, [m-theme="light"] {' + styleStringLight;
    ...
    styleStringDark += `--md-sys-color-primaryA12:` + "#" + addAlphaToColor((parseInt(themeDark.primary) & 0x00FFFFFF).toString(16).padStart(6, '0'), 0.12) + ';\n';
    styleStringDark += `--md-sys-color-onSurfaceA12:` + "#" + addAlphaToColor((parseInt(themeDark.onSurface) & 0x00FFFFFF).toString(16).padStart(6, '0'), 0.12) + ';\n';
